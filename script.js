...
function updateRoomAssignmentTable() {
    ...
    let rooms = [...new Set([...students.map(s => s.room), ...schedule.map(s => s.room)])];
    rooms.forEach(room => {
        const studentNames = students.filter(s => s.room === room).map(s => s.name).join("\n") || "None";
        const teacherNames = schedule.filter(s => s.room === room).map(s => s.teacher).join(", ") || "None";
        const row = table.insertRow();
